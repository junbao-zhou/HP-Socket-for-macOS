...
public:
    ...
    struct CallFuncInfo{
        ...
        CallFuncInfo(T* pRunner, F pFunc, P* pArg, PVOID pThis):
            m_pRunner(pRunner), m_pFunc(pFunc), m_pArg(pArg), m_pThis(pThis){
...
public:
	...
	BOOL Start(T* pRunner, F pFunc, P* pArg = nullptr, const pthread_attr_t* pAttr = nullptr)
	{
		...
		else
		{
            ...
            rs = pthread_create(&m_ulThreadID, pAttr, ThreadProc, (PVOID)(new CallFuncInfo(m_pRunner, m_pFunc, m_pArg, this)));
            SetRunning(TRUE);
...
private:
	static PVOID ThreadProc(LPVOID pv)
    {
        ...
        CallFuncInfo* pInfo = (CallFuncInfo*)pv;
        T* pRunner = pInfo->m_pRunner;
        F pFunc = pInfo->m_pFunc;
        P* pArg = pInfo->m_pArg;
        CThread* pThis		= (CThread*)pInfo->m_pThis;
        delete pInfo;
        pv = pInfo = nullptr;

		pThis->m_lNativeID	= SELF_NATIVE_THREAD_ID;

        return pThis->Run((T*)nullptr, (R*)nullptr, pRunner, pFunc, pArg);
	}
    ...
    PVOID Run(T_*, R_*, T* pRunner, F pFunc, P* pArg)
	{
        return (PVOID)(UINT_PTR)((pRunner->*pFunc)(pArg));
	}

	template<typename T_, typename = enable_if_t<!is_same<T_, __CFakeRunnerClass_>::value>>
    PVOID Run(T_*, PVOID, T* pRunner, F pFunc, P* pArg)
	{
        (pRunner->*pFunc)(pArg);
    ...
    PVOID Run(__CFakeRunnerClass_*, R_*, T* pRunner, F pFunc, P* pArg)
	{
        return (PVOID)(UINT_PTR)(*(SF*)&pFunc)(pArg);
	}

    PVOID Run(__CFakeRunnerClass_*, VOID*, T* pRunner, F pFunc, P* pArg)
	{
        (*(SF*)&pFunc)(pArg);
