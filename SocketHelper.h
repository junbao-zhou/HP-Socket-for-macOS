...
/* TCP 默认心跳确认包检测间隔 */
#define DEFALUT_TCP_KEEPALIVE_INTERVAL			(30 * 1000)
...
typedef struct hp_sockfamily{
private:
    __uint8_t	sin_len;
public:
    ADDRESS_FAMILY	family;

    hp_sockfamily& operator =(int family){
        this->family = family;
        return *this;
    }

    bool operator ==(int family) const{
...
typedef struct hp_sockaddr
{
    ...
    inline static int AddrSize(HP_FAMILY f)
	{
		if(f == AF_INET)
