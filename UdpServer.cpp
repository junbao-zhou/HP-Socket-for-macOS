...
BOOL CUdpServer::HandleClose()
{
    ...
    m_ioDispatcher.CtlFD(m_soListen, EV_DELETE, 0, &m_soListen);
	return FALSE;
}
...
BOOL CUdpServer::HandleReceive(int flag)
{
	...
	{
		HP_SOCKADDR addr;
		DWORD dwAddrLen = (DWORD)addr.AddrSize();
...
{
	TRACE("<S-CNNID: %Iu> recv 0 bytes (detect package)", pSocketObj->connID);
	...
	TRACE("<S-CNNID: %Iu> send 0 bytes (detect ack package - %s)", pSocketObj->connID, IS_HAS_ERROR(rc) ? "fail" : "succ");
}
...
BOOL CUdpServer::DoReceive(CONNID dwConnID, int flag)
{
	...
	for(int i = 0; i < reads || reads < 0; i++)
	{
		...
		{
			TRACE("<S-CNNID: %Iu> OnReceive() event return 'HR_ERROR', connection will be closed !", dwConnID);
...
{
    //读取数据，此模型非常适合EV_CLEAR而不适合EV_DISPATCH
    //m_ioDispatcher.CtlFD(m_soListen, EV_ADD | EV_CLEAR | EV_ENABLE, EVFILT_READ, &m_soListen);
...
BOOL CUdpServer::DoSend(CONNID dwConnID, int flag)
{
	...
	for(int i = 0; i < writes || writes < 0; i++)
	{
		...
		if(bBlocked)
		{
            ...
            //发送数据，则适合EV_DISPATCH
            m_ioDispatcher.CtlFD(m_soListen, EV_ADD | EV_DISPATCH | EV_ENABLE, EVFILT_WRITE, &m_soListen);
			break;
		}
	}
