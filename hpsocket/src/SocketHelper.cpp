...
int SSO_KeepAliveVals(SOCKET sock, BOOL bOnOff, DWORD dwIdle, DWORD dwInterval, DWORD dwCount)
{
	...
	if(isOK && bOnOff)
    {
        ...
        //TCP_KEEPINTVL
        //TCP_KEEPCNT
        //设置需要在外部设置，在软件内部设置会导致监听套接字自动关闭。
        /*
            //http://www.voidcn.com/article/p-afuibcmg-bqk.html
            //Mac上当设置，keepalive触发时间、间隔时间和探测计数会导致listen socket自动失效
            isOK &= IS_NO_ERROR(setsockopt(sock, IPPROTO_TCP, TCP_KEEPALIVE, &dwIdle, sizeof(DWORD)));
            isOK &= IS_NO_ERROR(setsockopt(sock, IPPROTO_TCP, TCP_KEEPINTVL, &dwInterval, sizeof(DWORD)));
            isOK &= IS_NO_ERROR(setsockopt(sock, IPPROTO_TCP, TCP_KEEPCNT, &dwCount, sizeof(DWORD)));
        */
        printf("Don't setting tcp keepalive time & keepintvl & keepcnt\n");
#else
